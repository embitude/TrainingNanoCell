					 ...
					 int rx_buf_sz)
{
	struct sk_buff *skb;
	dma_addr_t mapping;
	//TODO: Allocate the skb
	skb = dev_alloc_skb(rx_buf_sz + RTK_RX_ALIGN);
	if (!skb)
		return -ENOMEM;
	//TODO: Make the space for skb_reserve
	skb_reserve(skb, RTK_RX_ALIGN);
	*sk_buff = skb;
	//TODO: Get the mapping for DMA
	mapping = pci_map_single(pdev, skb->data, rx_buf_sz,
							 PCI_DMA_FROMDEVICE);
	//TODO: Mark it to the device	
...
static uint32_t rtl8101_rx_fill(DrvPvt *pvt, uint32_t start, uint32_t end)
{
	...
	for (cur = start; end - cur > 0; cur++) {
		...
		if (pvt->rx_skbuff[i])
			continue;
		//TODO: Allocate the skb with rtl8101_alloc_rx_skb
...
static int rtl8101_init_buffers(DrvPvt *pvt)
{
	 ...
	 */
	//TODO 6: Allocate the consistent memory for TX Descriptor Array
	pvt->TxDescArray = pci_alloc_consistent(pdev, R8101_TX_RING_BYTES,
											&pvt->TxPhyAddr);
	if (!pvt->TxDescArray)
		return -ENOMEM;

	//TODO 7: Allocate the consistent memory for RX Descriptor Array
	...
	//TODO 8: Initialize the various indices for tracking the packets
					 ...
					 void __iomem *reg_base, uint32_t budget)
{
	...
	for (; rx_left > 0; rx_left--, cur_rx++) {
		...
		if (unlikely(status & RxRES)) {
			...
			//TODO 22: Get the packet size from opts 1 field
			int pkt_size = (status & 0x00003FFF) - 4;
			//TODO 23: Unamp the skbuff
			pci_unmap_single(pvt->pci_dev,
							 le64_to_cpu(desc->addr), pvt->rx_buf_sz,
							 PCI_DMA_FROMDEVICE);
			pvt->rx_skbuff[entry] = NULL;
			skb->dev = dev;
			//TODO 24: Specify the packet size in skb
	...
	//TODO 25: Allocate the skbs for receive 
...
static int rtl8101_poll(struct napi_struct *napi_ptr, int budget)
{
	...
	if (work_done < budget) {
		//TODO 26: Mark the napi complete and enable the interrupts
...
static irqreturn_t rtl8101_interrupt(int irq, void *dev_instance)
{
	...
	//TODO 19: Disable & Clear the interrupts
	rtl8101_disable_n_clear_interrupts(reg_base);
	//TODO 20: Schedule the polling
...
{
	//TODO 5: Get the private data & Get reg_base and pci_dev
	DrvPvt *pvt = netdev_priv(dev);
	void __iomem *reg_base = pvt->reg_base;
	struct pci_dev *pdev = pvt->pci_dev;
	int retval;
	...
	napi_enable(&pvt->napi);
	rtl8101_hw_reset(reg_base);
	//TODO 10: Power up the phy
	phy_power_up(reg_base);
	rtl8101_hw_init(dev);
	//TODO 12: REgister the interrupt handler
...
{
	DrvPvt *pvt = netdev_priv(dev);
	//TODO 13: Get the current index for TX descriptor
	unsigned int entry = pvt->cur_tx % NUM_TX_DESC;
	//TODO 14: Get the current descriptor
...
int net_register_dev(struct pci_dev *pdev)
{
	...
	//TODO 1: Allocate the netdev with alloc_etherdev
