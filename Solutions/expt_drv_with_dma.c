...
//Refer Section 9.1.1, P-50 of datasheet
...
static int setup_buffers(struct pci_dev *dev)
{
    ...
    /* TODO 2: Allocate the coherent memory for tx descriptor */
	dpv->desc_tx = pci_alloc_consistent(dev, sizeof(Desc), &dpv->phy_desc_tx);
	if (!dpv->desc_tx)
		return -ENOMEM;
    /* TODO 3: Allocate the coherent memory for rx descriptor */
    ...
    /* TODO 6: Map receive buffer for performing the dma */
    ...
    /* TODO 7: Update the TX_DESC_ADDR_LOW_REG and TX_DESC_ADDR_HIGH_REG */
...
static irqreturn_t expt_pci_intr_handler(int irq, void *dev)
{
    ...
    /* TODO 9: check if TX_OK_BIT is set */
...
static ssize_t expt_write(struct file *f, const char __user *buf, size_t len, loff_t *off)
{
	...
	/* TODO 8: Make the buffer available to hardware and trigger transmit */
...
static int expt_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
    ...
    /* TODO 1: Enable the PCI device to become the bus master */
