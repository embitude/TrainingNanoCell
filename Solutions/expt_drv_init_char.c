...
#define EXPT_VENDOR_ID PCI_VENDOR_ID_INTEL
#define EXPT_PRODUCT_ID 0x100f /* Fast ethernet card on PCs */
...
u16 read_eeprom(struct dev_priv *dpvt, u32 eaddr)
{
	u32 temp;
	temp = (u32)(1) | (eaddr << 8);
	iowrite32(temp, (dpvt->reg_base + 0x14));
	while (!(ioread32(dpvt->reg_base + 0x14) & (1 << 4)))
		udelay(1);
	temp = ioread32(dpvt->reg_base + 0x14);
...
static int expt_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
     ...
     */
	if ((dpv->reg_base = ioremap(pci_resource_start(dev, 0), pci_resource_len(dev, 0))) == NULL)
	{
	...
	addr = read_eeprom(dpv, 0);
	printk("Mac addr 0 - %x\n", addr);
	addr = read_eeprom(dpv, 1);
	printk("Mac addr 1 - %x\n", addr);
	addr = read_eeprom(dpv, 2);
	printk("Mac addr 2 - %x\n", addr);
