...
void rtl8101_get_mac_address(struct net_device *dev)
{
	...
	dev->dev_addr[0] = REG_R8(MAC0 + 0);
	dev->dev_addr[0] += 1;
	REG_W8(dev->dev_addr[0], MAC0 + 0);
...
static void rtl8101_desc_addr_fill(DrvPvt *pvt)
{
	...
	if (!pvt->TxPhyAddr || !pvt->RxPhyAddr)
		return;
	//TODO 10: Write the TX and RX descriptor address to Registers
